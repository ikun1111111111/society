...
/* ============================================================
 * 象罔社团官网 · 主脚本
 * 模块：粒子星域 / 打字机 / 跑马灯 / 3D 倾斜 / 数字滚动 / 滚动入场 / 进度条
 * ============================================================ */
    ...
    for (let i = 0; i < this.particles.length; i++) {
      for (let j = i + 1; j < this.particles.length; j++) {
        ...
        if (d2 < this.linkDist * this.linkDist) {
          const alpha = (1 - Math.sqrt(d2) / this.linkDist) * 0.22;
...
/** 无限跑马灯：内容复制一份实现无缝循环 */
function buildMarquee(el: HTMLElement, items: string[]): void {
  const half = items.map((s) => `<span><b>▸</b> ${s}</span>`).join('');
  el.innerHTML = half + half;
}

/** 3D 倾斜卡片（跟随鼠标），并写入 --mx/--my 供光晕定位 */
function bindTilt(scope: ParentNode = document): void {
...
/** 数字滚动：进入视口后从 0 数到目标值 */
function bindCounters(scope: ParentNode = document): void {
...
function main(): void {
  ...
  if (marquee) {
    buildMarquee(marquee, [
      'TypeScript', 'Vue', 'React', 'Node.js', 'Python',
      'Docker', 'Git', 'Linux', 'LLM / RAG', 'CTF', 'ACM',
    ]);
  }
