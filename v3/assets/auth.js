...
(function (w, d) {
   ...
   *
   *   DEMO_FORCE 控制它在哪些地址下生效：
   *     true  = 任何地址都可用（线上也能用一键进入）
   *     false = 只在本地地址可用（localhost / 127.0.0.1 / ::1 / file://），
   *             公网域名或 IP 访问时自动失效，登录页的相关入口也会被摘掉。
   *
   *   ⚠️ 现状是 true（用户明确要求「随时能进的测试账号」）。
   *   注意：口令写死在源码里，仓库又是公开的 —— 谁都能看到。
   *   本仓库里本来就有可无限次使用的管理员邀请码（XW2026-DEMO / XW2026-ADMIN），
   *   所以公开这个口令并不额外增加多少风险；真正的边界是「别往站上放敏感数据」。
   *   想收紧就把这里改成 false 重新部署。
   * ============================================================ */
  var DEMO_FORCE = true;
